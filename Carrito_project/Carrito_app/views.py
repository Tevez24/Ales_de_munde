...
from .forms import CustomUser_CreationForm, EditProfileForm
        ...
        if form.is_valid():
            ...
            try:
                ...
                if not user_check.is_active:
                    messages.error(request, "Tu cuenta no ha sido activada. Por favor, revisa tu correo electrÃ³nico y completa el proceso de verificaciÃ³n.")
                ...
                if user is not None:
                    ...
                    try:
                        if user.email:
                            send_mail(
                                subject='Inicio de sesiÃ³n exitoso - Ailes du Monde',
                                message=f'Hola {user.username},\n\nIniciaste sesiÃ³n el {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}.\nGracias por usar Ailes du Monde!',
                ...
                messages.error(request, "No se encontrÃ³ ninguna cuenta con ese nombre de usuario. Por favor, verifica tus datos o regÃ­strate.")
        else:
            messages.error(request, "Por favor, introduce un nombre de usuario y contraseÃ±a vÃ¡lidos.")
    else:
...
def register_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Store user_id and code in session, with an expiry
            ...
            try:
                send_mail(
                    subject='Tu cÃ³digo de verificaciÃ³n - Ailes du Monde',
                    message=f'Hola {user.username},\n\nTu cÃ³digo de verificaciÃ³n es: {verification_code}\n\nGracias por registrarte en Ailes du Monde!',
                    from_email=settings.EMAIL_HOST_USER,
                    recipient_list=[user.email],
                    fail_silently=False,
                )
                messages.success(request, "Â¡Registro exitoso! Te hemos enviado un cÃ³digo de verificaciÃ³n a tu correo.")
                return redirect('verify_code') 
            except Exception as e:
                print(f'Error al enviar correo: {e}')
                messages.error(request, "No se pudo enviar el correo de verificaciÃ³n. Contacta a soporte.")
    else:
...
def verify_code(request):
    if 'user_id_for_verification' not in request.session:
        messages.error(request, "Tu sesiÃ³n de verificaciÃ³n ha expirado o es invÃ¡lida. Por favor, regÃ­strate o inicia sesiÃ³n de nuevo.")
    ...
    if request.method == 'POST':
        ...
        if entered_code and entered_code == verification_code:
            ...
            messages.success(request, "Â¡VerificaciÃ³n exitosa! Has iniciado sesiÃ³n.")
            return redirect('home')
        else:
            messages.error(request, "El cÃ³digo de verificaciÃ³n es incorrecto. Por favor, intÃ©ntalo de nuevo.")
...
def add_to_cart(request, paquete_id):
    ...
    request.session.modified = True
    return JsonResponse({'status': 'success', 'message': 'Paquete aÃ±adido al carrito.'})
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        request.session.modified = True
        messages.success(request, "Â¡Compra realizada con Ã©xito! Se descargarÃ¡ tu comprobante.")
...
def order_history(request):
    # This is a placeholder. You should implement logic to retrieve actual orders.
    orders = [] 
